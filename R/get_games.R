...
#' @export
#'
get_games <- function(start_date_range = NULL, end_date_range = NULL, team = NULL, conference = NULL, season = NULL, season_type = NULL, status = NULL, tournament = NULL) {
  ...
  if (!is.null(status)) stopifnot(status %in% c("scheduled", "in_progress", "final", "postponed", "cancelled") & is.character(status))
  if (!is.null(tournament)) stopifnot(is.character(tournament))
  # perform api call
  query_cbbd("games", list("startDateRange" = start_date_range, "endDateRange" = end_date_range, "team" = team, "conference" = conference, "season" = season, "seasonType" = season_type, "status" = status, "tournament" = tournament))
}
